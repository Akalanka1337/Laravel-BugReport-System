...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetLink(Request $request, Admins $Admins)
    {
        ...
        if ($email) {
            ...
            \Mail::to($email->email)->send(new \App\Http\Controllers\Dashboard\Auth\EmailResetLink($data));
            return redirect()->back()->with('message_sent', trans('global.reset_email_send_success'));
        } else {
    ...
    public function showResetForm(Request $request, Admins $Admins)
    {
        ...
        if ($token) {
            ...
            return view('dashboard.auth.passwords.reset')->with(compact('reset_token'));
        }
