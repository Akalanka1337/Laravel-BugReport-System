...
@section('backBtn')
    <a href="{{ route("dashboard.admins.index") }}"><i class="fas fa-angle-left"></i> {{ trans('global.page_back_btn') }}</a>
@endsection
@section('content')
        ...
        <form method="POST" action="{{ $admiko_data['formAction'] }}" enctype="multipart/form-data" class="needs-validation" novalidate>
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" id="name" name="name" required placeholder="{{ trans('global.admins_name') }}" value="{{{ old('name', isset($data)?$data->name : '') }}}">
                        @if ($errors->has('name'))
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="email" class="form-control" id="email" name="email" required placeholder="{{ trans('global.admins_email') }}" value="{{{ old('email', $data->email??'') }}}">
                        @if ($errors->has('email'))
                ...
                <div class="form-group row">
                    <label for="role_id" class="col-sm-2 col-form-label">{{ trans('global.admins_role') }}:</label>
                    <div class="col-sm-10">
                ...
                <div class="form-group row">
                    <label for="access" class="col-sm-2 col-form-label">{{trans('global.multi_tenancy_access_title')}}</label>
                    <div class="col-sm-10">
                        @if(isset($data) && $data->role_id != 1)
                            ...
                            @endforeach
                            <div class="invalid-feedback @if ($errors->has('access')) d-block @endif">{{trans('global.required_text')}}</div>
                        @else
                            <p class="text-muted pt-2">{{trans('global.multi_tenancy_access_limit')}}</p>
                        @endif
                        <small id="access_help" class="text-muted pt-2">{{trans('global.multi_tenancy_access')}}</small>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-actions">
                    <div class="row" id="form-group-buttons">
                        ...
                        <div class="secondaryButtons col pt-0 text-end">
                            <a href="{{ route("dashboard.admins.index") }}" class="btn btn-secondary mb-1 mb-sm-0  ms-3 ms-sm-0" role="button">{{ trans('global.table_cancel') }}</a>
                        </div>
                    </div>
                </div>
            </div>
