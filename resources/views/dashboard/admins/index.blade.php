    ...
    <div class="card">
            ...
            <div class="tableBox">
                <div class="row">
                    <div class="col-auto mb-2">
                        <a href="{{route('dashboard.admin_roles.index')}}"><i class="fas fa-cog fa-fw"></i> {{ trans('global.roles_manager') }}</a>
                    </div>
                    ...
                    <table class="table" data-dom="tr" data-page-length='-1'>
                        ...
                        @foreach($tableData as $data)
                            <tr>
                                ...
                                <td class=" nowrap"><a href="{{route('dashboard.admins.edit',$data->id)}}">{{{$data->name}}}</a></td>
                                <td class=" nowrap"><a href="{{route('dashboard.admins.edit',$data->id)}}">{{{$data->email}}}</a></td>
                                <td class=" nowrap">{{$data->adminsRole->title??''}}</td>
                                <td class="w-5 no-sort">
                                    <a href="{{route('dashboard.admins.edit',$data->id)}}"><i class="fas fa-edit fa-fw"></i></a>
                                </td>
                                <td class="w-5 no-sort">
                                    @if($data->id != 1)
                                        <a href="{{route('dashboard.admins.destroy',$data->id)}}" data-id="{{$data->id}}" class="admiko_deleteConfirm" data-bs-toggle="modal" data-bs-target="#deleteConfirm"><i class="fas fa-trash fa-fw"></i></a>
                                    @endIf
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="row">
                    <div class="col-12 col-sm order-3 order-sm-0 pt-3">
                        <a href="{{route('dashboard.admins.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{ trans('global.table_add') }}</a>
                    </div>
        ...
        <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
            <div class="modal-dialog">
                <form method="post" action="{{route("dashboard.admins.delete")}}">
                    @method('DELETE')
                    @csrf
